...
module.exports = (config, context) => {
  ...
  config.plugins.push(copyNodeVerionFile(outputPath));
...
function copyPackageLockFile(outputPath) {
  return new CopyPlugin({
    patterns: [{ from: 'package-lock.json', to: join(outputPath, 'package-lock.json') }],
  });
}
...
/**
 * Copy the .nvmrc file to the bundle t maoke sure that the right node version is being used.
 *
 * @param {String} outputPath The path to the bundle being built
 * @returns {*} A Webpack plugin
 */
function copyNodeVerionFile(outputPath) {
  return new CopyPlugin({
    patterns: [
      {
        context: './',
        from: '.nvmrc',
        to: join(outputPath, '.nvmrc'),
        toType: 'file',
      },
    ],
  });
